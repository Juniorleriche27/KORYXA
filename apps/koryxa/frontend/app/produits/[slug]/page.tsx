...
export default async function ProductPage(props: ProductPageProps) {
  ...
  return (
    <main>
      <section className="relative w-full bg-[radial-gradient(ellipse_at_70%_50%,#245b37_0%,#14351f_55%,#07140c_100%)] py-16 text-white sm:py-20">
        <div className="mx-auto max-w-[var(--marketing-max-w)] px-4 sm:px-6 lg:px-8">
          <div className="mb-6 flex flex-col items-start gap-4 sm:flex-row sm:items-center">
            ...
            <div>
              ...
              <p className="mt-2 text-base text-[#f1d57a] sm:text-lg">{product.tagline}</p>
            </div>
          </div>
          <p className="max-w-3xl text-base text-[#f1d57a] sm:text-xl">{product.summary}</p>
        </div>
      </section>
      ...
      <section className="bg-white px-4 py-20 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-[var(--marketing-max-w)] gap-12 md:grid-cols-2">
          ...
          <article className="rounded-[30px] border border-slate-200/80 bg-slate-50 p-8 shadow-[0_18px_46px_rgba(15,23,42,0.06)]">
            ...
            <Link href={product.primaryCta.href} className="mt-6 inline-flex w-full items-center justify-center rounded-xl bg-[#14351f] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#245b37]">
              {product.primaryCta.label}
            </Link>
            {product.secondaryCta ? (
              <Link href={product.secondaryCta.href} className="mt-3 inline-flex w-full items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-[#c9a227] hover:text-[#14351f]">
